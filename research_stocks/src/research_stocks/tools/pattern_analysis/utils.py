...
from tools.s3_gpt_analysis import load_grok_parse_json_instructions
...
def get_pattern_reliability(name: str | None = None):
  ...
  reliability = {"Head and Shoulders": 0.75, "Inverse Head and Shoulders": 0.75,
    "Double Top": 0.70, "Double Bottom": 0.70, "Triple Top": 0.80,
    "Triple Bottom": 0.80, "Ascending Triangle": 0.65,
    "Descending Triangle": 0.65, "Symmetrical Triangle": 0.60,
    "Rising Wedge": 0.60, "Falling Wedge": 0.60, "Channel Up": 0.55,
    "Channel Down": 0.55, }
...
def run_grok_on_fintech_block(block: dict, *, symbol: str, timeframe: str,
    model: str | None = None, endpoint: str | None = None) -> dict:
  ...
  messages = load_grok_parse_json_instructions(block, symbol, timeframe)

  # Create xAI client using OpenAI SDK and API key
  client = OpenAI(
  ...
  try:
    ...
    logging.warning("Grok call failed for %s %s: %s", symbol, timeframe, e)
    # Fail soft so your pipeline continues
    return {"timeframe": timeframe, "symbol": symbol, "summary": "llm_error", "bias": "neutral",
      "signals": [], "levels": {"support": [], "resistance": []},
      "setups": [], }
