...
def get_implied_move(symbol: str) -> dict:
  ...
  price = float(
      calls.loc[calls_idx, "lastPrice"] + puts.loc[puts_idx, "lastPrice"])
  return {"absolute": round(price, 2),
          "percent": round(price / last_close * 100, 2)}
...
def get_opening_microstructure(symbol: str, date: datetime) -> dict:
  ...
  return {"auction_imbalance_shares": int(imbalance_shares),
...
def get_prior_close_microstructure(symbol: str, now: datetime) -> dict:
  ...
  # ➋  Slice 14:00-16:00 ET
  df = df.between_time("14:00", "16:00")
  if df.empty:
    raise RuntimeError("No bars in the 14-16 ET window")
...
def get_active_catalysts(symbol: str, date: datetime) -> List[dict]:
  ...
  # ── normalise to a DataFrame with events in rows ─────────────────────
  if isinstance(cal_raw, pd.DataFrame):
    cal = cal_raw.T
  elif isinstance(cal_raw, dict):
    cal = pd.DataFrame(cal_raw).T
  else:
    raise RuntimeError("Unknown calendar data structure")

  # earnings date may be 'Earnings Date' or 'earningsDate'
  earnings_row = cal.loc[cal.index.str.contains("earnings", case=False)]
  if not earnings_row.empty:
    earnings_ts = earnings_row.iloc[0, 0]
    if pd.notna(earnings_ts):
      when = pd.to_datetime(earnings_ts).tz_localize(None)
...
def get_headline_sentiment(symbol: str) -> dict:
  ...
  return {"dominant": dominant, "score": round(avg, 2),
...
def get_confirmed_patterns(symbol: str) -> List[dict]:
  ...
  for p in results.get("patterns", []):
    ...
    if end >= cutoff:
      patterns.append(
          {"name": p.get("pattern"), "direction": p.get("direction"),
...
def collect_intraday_factors(symbol: str, output_dir: Path) -> Path:
  ...
  data = {"symbol": symbol.upper(),
          ...
          "prior_close_microstructure": get_prior_close_microstructure(symbol,
                                                                       now),
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser(
      description="Collect intraday forecast inputs")
