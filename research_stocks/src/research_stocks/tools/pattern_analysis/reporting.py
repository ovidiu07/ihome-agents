...
def export_analysis_results(results: Dict[str, Any], output_dir: str = "output") -> None:
    """
    ...
    # Save to JSON file
    output_file = os.path.join(output_dir, "pattern_analysis_results.json")
...
def generate_evolving_daily_ohlc(intraday_df: pd.DataFrame) -> Dict[str, float]:
    ...
    date_str = intraday_df.iloc[0]['Date'].split(' ')[0] if ' ' in intraday_df.iloc[0]['Date'] else intraday_df.iloc[0]['Date']
